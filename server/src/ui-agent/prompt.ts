import { ChatPromptTemplate, HumanMessagePromptTemplate,BaseStringPromptTemplate, PromptTemplate, SystemMessagePromptTemplate} from "langchain/prompts";
...
const messagePrompt = new PromptTemplate({
    ...
    template:"Modify the code based on the users requests, and the existing code state.",
...
const promptBuilder = ChatPromptTemplate.fromPromptMessages([
    SystemMessagePromptTemplate.fromTemplate(
      `You are a UI design tool which writes and edits html, css, and js code based on user requests.
      Given user input, and the existing state of the code files, you will rewrite the code.`
    ),
  ...
  promptBuilder.partialVariables = {format_instructions: parser.getFormatInstructions()};
